...
export default function EditBlogPage() {
	...
	if (loading) {
		return <div className='text-secondary flex h-screen items-center justify-center text-sm'>{t('blog.loading')}</div>
	}

	if (!slug) {
		return <div className='flex h-screen items-center justify-center text-sm text-red-500'>{t('blog.invalidId')}</div>
	}
